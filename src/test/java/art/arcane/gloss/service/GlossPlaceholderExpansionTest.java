...
public class GlossPlaceholderExpansionTest {
  ...
  public void menuOpen_followsThePublishedSessionForThatPlayerOnly() {
    ...
    PlaceholderKeyRegistry registry = GlossPlaceholderExpansion.registry(openMenus);
  ...
  public void expansionPublishesExactlyThreeKeysAndPapiSafeMetadata() {
    PlayerSnapshotStore<String> openMenus = new PlayerSnapshotStore<>();
    VolmitPlaceholderExpansion expansion = new GlossPlaceholderExpansion(openMenus, Logger.getAnonymousLogger());

    assertEquals("gloss", expansion.getIdentifier());
    assertEquals("Gloss", expansion.getRequiredPlugin());
