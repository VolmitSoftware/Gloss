...
/**
 * Registry behaviour that never touches Bukkit: extraction, loading, reload semantics, match
 * resolution, and the raycast eligibility set. {@code startWatching} is deliberately not exercised
 * here — it only arms the shared {@code DataWatchdog} entry, and the constructor performs the whole
 * load, so every rule below is reachable by calling {@link PreviewDocumentRegistry#reload()}
 * directly, which is exactly what the watchdog pass does once a file changes.
 *
...
public class PreviewRegistryTest {
  ...
  /**
   * {@code normalize} is public so {@code CommandGlossPreview.dump} can give a name arg the same
  ...
  @Test
  public void stoppingAnUnarmedWatchIsHarmlessWithoutAPluginInstance() {
    PreviewDocumentRegistry registry = new PreviewDocumentRegistry(configDir);

    registry.stopWatching();
    registry.stopWatching();

    assertEquals(PreviewDocumentRegistry.SHIPPED.size(), registry.names().size());
  }
