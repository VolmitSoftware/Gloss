...
public final class LegacyHologramImportService {
  ...
  private ExistingSnapshot snapshotExisting() {
    ...
    for (PanelDefinition board : boardService.list()) {
  ...
  private PanelService requireBoardService() {
    PanelService boardService = plugin.getPanelService();
  ...
  private static ThreadFactory threadFactory(ClassLoader classLoader) {
    return task -> {
      Thread thread = new Thread(task, "Gloss-Legacy-Hologram-Scanner");
