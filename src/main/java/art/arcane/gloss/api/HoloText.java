...
final class HoloText {
  ...
  static String sanitizePath(String value) {
    ...
    if (normalized.isEmpty() || normalized.startsWith("/") || normalized.indexOf(':') >= 0) {
      throw new IllegalArgumentException("image path must be relative to the Gloss images folder (plugins/Gloss/images/): " + value);
    }

    for (String segment : normalized.split("/")) {
      if (segment.equals("..")) {
        throw new IllegalArgumentException("image path must not escape the Gloss images folder (plugins/Gloss/images/): " + value);
      }
    }
