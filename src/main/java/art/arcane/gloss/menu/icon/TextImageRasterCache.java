...
/**
 * Shared rasters for the two image icons. Turning an image into text lines allocates two Components
 * per pixel, and every session that opens the same menu used to redo it; because Components are
 * immutable the finished lines can simply be shared.
 *
...
final class TextImageRasterCache {
  ...
  static Component blankRow(int width) {
    return BLANK_ROWS.computeIfAbsent(width, pixels -> {
      TextComponent.Builder row = Component.text();
      for (int x = 0; x < pixels; x++) {
        appendTransparent(row);
      }
      return row.build();
    });
  }
  ...
  private static List<Component> build(int[] pixels, int width, int height, boolean forceOpaque) {
    ...
    for (int y = 0; y < height; y++) {
      ...
      for (int x = 0; x < width; x++) {
        ...
        if (!forceOpaque && ((colour >> 24) & 0x0000FF) < 255) {
          appendTransparent(line);
        } else {
          line.append(TextUtils.textColor("█", colour & 0x00FFFFFF));
        }
      }
  ...
  private static void appendTransparent(TextComponent.Builder line) {
    line.append(Component.text(" ").decorate(TextDecoration.BOLD)).append(Component.text(" "));
  }
